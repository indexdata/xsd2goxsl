    ...
    <xsl:choose>
      <xsl:when test="$level != '' and $parentPtr != ''">
        ...
        <xsl:call-template name="emit-field-tags">
          <xsl:with-param name="xml" select="string($xmlTag)"/>
          <xsl:with-param name="json" select="string($jsonTag)"/>
          <xsl:with-param name="ptr" select="$parentPtr"/>
  ...
  <xsl:template name="emit-field-tags">
    ...
    <xsl:if test="$validate = 'yes'">
      <xsl:variable name="rules">
        <xsl:call-template name="build-validate-rules">
      ...
      <xsl:if test="string($rules) != ''">
        ...
        <xsl:value-of select="string($rules)"/>
      </xsl:if>
    </xsl:if>
    <xsl:text>"`</xsl:text>
  </xsl:template>

  <xsl:template name="build-validate-rules">
    <xsl:param name="xml"/>
    <xsl:param name="ptr"/>
    <xsl:param name="type"/>
    ...
    <xsl:variable name="stringLikeScalar">
      <xsl:call-template name="is-string-like-scalar">
        <xsl:with-param name="type" select="$localType"/>
      </xsl:call-template>
    </xsl:variable>
    <xsl:variable name="presenceRule">
      <xsl:if test="not(self::xs:attribute)
                    and $ptr = ''
                    and not(contains($xml, ',omitempty'))
                    and string($stringLikeScalar) = 'yes'">
    ...
    <xsl:variable name="valueRule">
      <xsl:if test="$ptr != '[]'">
        <xsl:value-of select="string($enumRule)"/>
      </xsl:if>
    </xsl:variable>
    <xsl:variable name="diveRule">
      <xsl:if test="$ptr = '[]' and string($needsDive) = 'yes'">
        <xsl:text>dive</xsl:text>
        <xsl:if test="string($enumRule) != ''">
          <xsl:text>,</xsl:text>
          <xsl:value-of select="string($enumRule)"/>
        </xsl:if>
      </xsl:if>
